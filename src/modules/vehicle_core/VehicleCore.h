...
public:
  ...
  void update(uint64_t tick, bool armed, const std::string &safety_mode, const LocalPosition &pos,
              const BatteryStatus &battery, const WpNavOutput &nav, const RangeFinderSample &rng, bool gcs_link,
              UORB &uorb);
...
private:
  ...
  void build_setpoints(const LocalPosition &pos, const WpNavOutput &nav, const RangeFinderSample &rng, double dt_s);
