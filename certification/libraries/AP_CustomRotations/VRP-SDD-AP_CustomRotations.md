...
| API | Description |
|---|---|
| `VRP_CustomRotations::init()` | Public API |
| `VRP_CustomRotations::update(const Attitude &att, bool armed)` | Public API |
| `VRP_CustomRotations::format_custom_rotations(const CustomRotationsState &s)` | Public API |

## 6. Verification

Tests: see LLRD `tests:` field and phase runner `test/libraries/run_phase20_tests.py`.
SVCP target: `test/libraries/VRP_CustomRotations/`
