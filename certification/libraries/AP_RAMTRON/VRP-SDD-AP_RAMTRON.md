...
| API | Description |
|---|---|
| `VRP_RAMTRON::init()` | Public API |
| `VRP_RAMTRON::status(uint64_t tick)` | Public API |
| `VRP_RAMTRON::format_ramtron(const RamtronState &s)` | Public API |

## 6. Verification

Tests: see LLRD `tests:` field and phase runner `test/libraries/run_phase17_tests.py`.
SVCP target: `test/libraries/VRP_RAMTRON/`
