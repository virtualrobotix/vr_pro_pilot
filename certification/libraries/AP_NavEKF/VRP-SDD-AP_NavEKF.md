...
| API | Description |
|---|---|
| `VRP_NavEKF::init()` | Public API |
| `VRP_NavEKF::update(const LocalPosition &pos, bool armed, uint64_t tick)` | Public API |
| `VRP_NavEKF::format_nav_ekf(const NavEkfState &s)` | Public API |

## 6. Verification

Tests: see LLRD `tests:` field and phase runner `test/libraries/run_phase1_tests.py`.
SVCP target: `test/libraries/VRP_NavEKF/`
