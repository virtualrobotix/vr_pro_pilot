...
| API | Description |
|---|---|
| `VRP_CheckFirmware::init()` | Public API |
| `VRP_CheckFirmware::verify(uint64_t tick)` | Public API |
| `VRP_CheckFirmware::format_check_firmware(const CheckFirmwareState &s)` | Public API |

## 6. Verification

Tests: see LLRD `tests:` field and phase runner `test/libraries/run_phase19_tests.py`.
SVCP target: `test/libraries/VRP_CheckFirmware/`
