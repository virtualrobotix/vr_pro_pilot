...
| API | Description |
|---|---|
| `VRP_Gripper::init()` | Public API |
| `VRP_Gripper::update(float command, bool armed)` | Public API |
| `VRP_Gripper::format_gripper(GripperState s)` | Public API |

## 6. Verification

Tests: see LLRD `tests:` field and phase runner `test/libraries/run_phase13_tests.py`.
SVCP target: `test/libraries/VRP_Gripper/`
