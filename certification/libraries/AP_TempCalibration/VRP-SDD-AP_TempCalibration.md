# VRP-SDD-AP_TempCalibration — Software Design Description

| Field | Value |
|---|---|
| Module | `VRP_TempCalibration` |
| LLRD | [`VRP-LLRD-AP_TempCalibration.yaml`](VRP-LLRD-AP_TempCalibration.yaml) |
| DAL | C |
| Status | planned |

## 1. Purpose

Clean-room BSD design for `VRP_TempCalibration`, functional parity with ArduPilot `AP_TempCalibration`.

## 2. Architecture

_TBD during implementation phase 3._

## 3. Data flow

_TBD — uORB topics / HAL interfaces._

## 4. Safety constraints (DAL C)

- Statement coverage 100%
- Requirements-based tests
- Design review

## 5. Interface summary

| API | Description |
|---|---|
| `VRP_TempCalibration::init()` | Module initialization |
| `VRP_TempCalibration::update()` | Periodic update |

## 6. Verification

See SVCP under `test/libraries/VRP_TempCalibration/` and LLRD test list.
