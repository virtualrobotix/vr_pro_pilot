...
| API | Description |
|---|---|
| `VRP_Torqeedo::init()` | Public API |
| `VRP_Torqeedo::update(float throttle, bool armed, float voltage_v)` | Public API |
| `VRP_Torqeedo::format_torqeedo(const TorqeedoState &s)` | Public API |

## 6. Verification

Tests: see LLRD `tests:` field and phase runner `test/libraries/run_phase15_tests.py`.
SVCP target: `test/libraries/VRP_Torqeedo/`
