...
| API | Description |
|---|---|
| `VRP_ExternalControl::init()` | Public API |
| `VRP_ExternalControl::update(bool armed, bool gcs_link)` | Public API |
| `VRP_ExternalControl::format_external_control(const ExternalControlState &s)` | Public API |

## 6. Verification

Tests: see LLRD `tests:` field and phase runner `test/libraries/run_phase20_tests.py`.
SVCP target: `test/libraries/VRP_ExternalControl/`
