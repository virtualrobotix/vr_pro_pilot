# VRP-SDD-AP_InternalError — Software Design Description

| Field | Value |
|---|---|
| Module | `VRP_InternalError` |
| LLRD | [`VRP-LLRD-AP_InternalError.yaml`](VRP-LLRD-AP_InternalError.yaml) |
| DAL | B |
| Status | planned |

## 1. Purpose

Clean-room BSD design for `VRP_InternalError`, functional parity with ArduPilot `AP_InternalError`.

## 2. Architecture

_TBD during implementation phase 3._

## 3. Data flow

_TBD — uORB topics / HAL interfaces._

## 4. Safety constraints (DAL B)

- MC/DC on safety paths
- No dynamic heap
- Bounded WCET
- MISRA subset

## 5. Interface summary

| API | Description |
|---|---|
| `VRP_InternalError::init()` | Module initialization |
| `VRP_InternalError::update()` | Periodic update |

## 6. Verification

See SVCP under `test/libraries/VRP_InternalError/` and LLRD test list.
