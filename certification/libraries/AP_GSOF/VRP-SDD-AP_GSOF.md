...
| API | Description |
|---|---|
| `VRP_GSOF::init()` | Public API |
| `VRP_GSOF::update(const GpsSample &gps)` | Public API |
| `VRP_GSOF::format_gsof(const GsofState &s)` | Public API |

## 6. Verification

Tests: see LLRD `tests:` field and phase runner `test/libraries/run_phase20_tests.py`.
SVCP target: `test/libraries/VRP_GSOF/`
