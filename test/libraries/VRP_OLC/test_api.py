#!/usr/bin/env python3
"""SVCP smoke test — VRP_OLC (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_OLC" / "VRP-LLRD-AP_OLC.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_OLC" / "VRP-SDD-AP_OLC.md"
    text = sdd.read_text()
