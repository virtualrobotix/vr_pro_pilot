#!/usr/bin/env python3
"""SVCP smoke test — VRP_DAL (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_DAL" / "VRP-LLRD-AP_DAL.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_DAL" / "VRP-SDD-AP_DAL.md"
    text = sdd.read_text()
