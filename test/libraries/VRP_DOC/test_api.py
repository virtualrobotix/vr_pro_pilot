#!/usr/bin/env python3
"""SVCP smoke test — VRP_DOC (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "doc" / "VRP-LLRD-doc.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "doc" / "VRP-SDD-doc.md"
    text = sdd.read_text()
