#!/usr/bin/env python3
"""SVCP smoke test — VRP_Param (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_Param" / "VRP-LLRD-AP_Param.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_Param" / "VRP-SDD-AP_Param.md"
    text = sdd.read_text()
