#!/usr/bin/env python3
"""SVCP smoke test — VRP_SITL (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "SITL" / "VRP-LLRD-SITL.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "SITL" / "VRP-SDD-SITL.md"
    text = sdd.read_text()
