#!/usr/bin/env python3
"""SVCP smoke test — VRP_Rally (WIRED)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_Rally" / "VRP-LLRD-AP_Rally.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_Rally" / "VRP-SDD-AP_Rally.md"
    text = sdd.read_text()
