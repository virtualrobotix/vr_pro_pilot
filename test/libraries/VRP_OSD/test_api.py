#!/usr/bin/env python3
"""SVCP smoke test — VRP_OSD (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_OSD" / "VRP-LLRD-AP_OSD.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_OSD" / "VRP-SDD-AP_OSD.md"
    text = sdd.read_text()
