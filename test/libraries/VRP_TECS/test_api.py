#!/usr/bin/env python3
"""SVCP smoke test — VRP_TECS (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_TECS" / "VRP-LLRD-AP_TECS.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_TECS" / "VRP-SDD-AP_TECS.md"
    text = sdd.read_text()
