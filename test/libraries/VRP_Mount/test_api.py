#!/usr/bin/env python3
"""SVCP smoke test — VRP_Mount (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_Mount" / "VRP-LLRD-AP_Mount.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_Mount" / "VRP-SDD-AP_Mount.md"
    text = sdd.read_text()
