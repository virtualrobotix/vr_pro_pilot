#!/usr/bin/env python3
"""SVCP smoke test — VRP_PID (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "PID" / "VRP-LLRD-PID.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "PID" / "VRP-SDD-PID.md"
    text = sdd.read_text()
