#!/usr/bin/env python3
"""SVCP smoke test — VRP_GPS (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_GPS" / "VRP-LLRD-AP_GPS.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_GPS" / "VRP-SDD-AP_GPS.md"
    text = sdd.read_text()
