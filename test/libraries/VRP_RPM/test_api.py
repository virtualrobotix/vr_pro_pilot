#!/usr/bin/env python3
"""SVCP smoke test — VRP_RPM (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_RPM" / "VRP-LLRD-AP_RPM.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_RPM" / "VRP-SDD-AP_RPM.md"
    text = sdd.read_text()
