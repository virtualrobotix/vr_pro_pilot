#!/usr/bin/env python3
"""SVCP smoke test — VRP_Radio (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_Radio" / "VRP-LLRD-AP_Radio.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_Radio" / "VRP-SDD-AP_Radio.md"
    text = sdd.read_text()
