#!/usr/bin/env python3
"""SVCP smoke test — VRP_HAL (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_HAL" / "VRP-LLRD-AP_HAL.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_HAL" / "VRP-SDD-AP_HAL.md"
    text = sdd.read_text()
