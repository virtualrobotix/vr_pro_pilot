#!/usr/bin/env python3
"""SVCP smoke test — VRP_EFI (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_EFI" / "VRP-LLRD-AP_EFI.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_EFI" / "VRP-SDD-AP_EFI.md"
    text = sdd.read_text()
