#!/usr/bin/env python3
"""SVCP smoke test — VRP_ADC (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_ADC" / "VRP-LLRD-AP_ADC.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_ADC" / "VRP-SDD-AP_ADC.md"
    text = sdd.read_text()
