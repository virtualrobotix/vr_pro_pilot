#!/usr/bin/env python3
"""SVCP smoke test — VRP_MSP (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_MSP" / "VRP-LLRD-AP_MSP.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_MSP" / "VRP-SDD-AP_MSP.md"
    text = sdd.read_text()
