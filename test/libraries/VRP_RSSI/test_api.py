#!/usr/bin/env python3
"""SVCP smoke test — VRP_RSSI (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_RSSI" / "VRP-LLRD-AP_RSSI.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_RSSI" / "VRP-SDD-AP_RSSI.md"
    text = sdd.read_text()
