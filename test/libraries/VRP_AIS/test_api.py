#!/usr/bin/env python3
"""SVCP smoke test — VRP_AIS (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_AIS" / "VRP-LLRD-AP_AIS.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_AIS" / "VRP-SDD-AP_AIS.md"
    text = sdd.read_text()
