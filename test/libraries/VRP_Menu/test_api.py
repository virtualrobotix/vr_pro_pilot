#!/usr/bin/env python3
"""SVCP smoke test — VRP_Menu (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_Menu" / "VRP-LLRD-AP_Menu.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_Menu" / "VRP-SDD-AP_Menu.md"
    text = sdd.read_text()
