#!/usr/bin/env python3
"""SVCP smoke test — VRP_JSON (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_JSON" / "VRP-LLRD-AP_JSON.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_JSON" / "VRP-SDD-AP_JSON.md"
    text = sdd.read_text()
