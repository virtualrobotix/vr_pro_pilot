#!/usr/bin/env python3
"""SVCP smoke test — VRP_DDS (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_DDS" / "VRP-LLRD-AP_DDS.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_DDS" / "VRP-SDD-AP_DDS.md"
    text = sdd.read_text()
