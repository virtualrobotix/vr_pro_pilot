#!/usr/bin/env python3
"""SVCP smoke test — VRP_IOMCU (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_IOMCU" / "VRP-LLRD-AP_IOMCU.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_IOMCU" / "VRP-SDD-AP_IOMCU.md"
    text = sdd.read_text()
