#!/usr/bin/env python3
"""SVCP smoke test — VRP_ROMFS (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_ROMFS" / "VRP-LLRD-AP_ROMFS.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_ROMFS" / "VRP-SDD-AP_ROMFS.md"
    text = sdd.read_text()
