#!/usr/bin/env python3
"""SVCP smoke test — VRP_ONVIF (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_ONVIF" / "VRP-LLRD-AP_ONVIF.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_ONVIF" / "VRP-SDD-AP_ONVIF.md"
    text = sdd.read_text()
