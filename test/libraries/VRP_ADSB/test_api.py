#!/usr/bin/env python3
"""SVCP smoke test — VRP_ADSB (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_ADSB" / "VRP-LLRD-AP_ADSB.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_ADSB" / "VRP-SDD-AP_ADSB.md"
    text = sdd.read_text()
