#!/usr/bin/env python3
"""SVCP smoke test — VRP_DAC (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_DAC" / "VRP-LLRD-AP_DAC.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_DAC" / "VRP-SDD-AP_DAC.md"
    text = sdd.read_text()
