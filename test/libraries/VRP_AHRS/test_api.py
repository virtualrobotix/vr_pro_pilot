#!/usr/bin/env python3
"""SVCP smoke test — VRP_AHRS (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_AHRS" / "VRP-LLRD-AP_AHRS.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_AHRS" / "VRP-SDD-AP_AHRS.md"
    text = sdd.read_text()
