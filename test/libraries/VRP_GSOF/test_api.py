#!/usr/bin/env python3
"""SVCP smoke test — VRP_GSOF (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_GSOF" / "VRP-LLRD-AP_GSOF.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_GSOF" / "VRP-SDD-AP_GSOF.md"
    text = sdd.read_text()
