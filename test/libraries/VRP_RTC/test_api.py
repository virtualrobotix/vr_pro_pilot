#!/usr/bin/env python3
"""SVCP smoke test — VRP_RTC (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_RTC" / "VRP-LLRD-AP_RTC.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_RTC" / "VRP-SDD-AP_RTC.md"
    text = sdd.read_text()
