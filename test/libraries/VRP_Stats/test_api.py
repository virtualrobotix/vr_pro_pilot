#!/usr/bin/env python3
"""SVCP smoke test — VRP_Stats (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_Stats" / "VRP-LLRD-AP_Stats.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_Stats" / "VRP-SDD-AP_Stats.md"
    text = sdd.read_text()
