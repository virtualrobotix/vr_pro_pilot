#!/usr/bin/env python3
"""SVCP smoke test — VRP_Math (BASELINE)."""
...
def test_cert_llrd_exists() -> None:
    llrd = ROOT / "certification" / "libraries" / "AP_Math" / "VRP-LLRD-AP_Math.yaml"
    assert llrd.exists(), f"Missing {llrd}"


def test_sdd_not_template() -> None:
    sdd = ROOT / "certification" / "libraries" / "AP_Math" / "VRP-SDD-AP_Math.md"
    text = sdd.read_text()
